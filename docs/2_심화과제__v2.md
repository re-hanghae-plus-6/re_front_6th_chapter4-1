...
**핵심 키워드:** server-safe, typeof window, useEffect branching
...
**핵심 키워드:** vite config, SSR externals, TypeScript
...
export default defineConfig({
  ...
  build: {
    rollupOptions: {
      external: ["react-dom/server"] // SSR 빌드 최적화
    }
  },
  ssr: {
    noExternal: ["@hanghae-plus/lib"] // 서버에서 번들링
  }
});
```
...
{
  "scripts": {
    "build:csr": "vite build --outDir ../../dist/react",
