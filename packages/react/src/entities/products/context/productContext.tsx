...
export const ProductProvider = ({ children, productStore }: Props) => {
  ...
  const setCategory = (categoryData: StringRecord) => {
  ...
  const loadProductDetailForPage = async (productId: string) => {
    try {
  ...
  const loadRelatedProducts = async (category2: string, excludeProductId: string) => {
    try {
      const params = {
        category2,
