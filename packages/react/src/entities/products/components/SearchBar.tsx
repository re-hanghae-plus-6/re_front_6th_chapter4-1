...
export function SearchBar() {
  const { action, state } = useProductStoreContext();

  const { searchQuery, limit = "20", sort, category } = useProductFilter();

  const categoryList = Object.keys(state.categories).length > 0 ? Object.keys(state.categories) : [];
  ...
  const handleBreadCrumbClick = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      if (breadcrumbType === "reset") {
        // "전체" 클릭 -> 카테고리 초기화
        action.setCategory({ category1: "", category2: "" });
        ...
        action.setCategory({ ...(category1 && { category1 }), category2: "" });
      }
    } catch (error) {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {/* 2depth 카테고리 */}
          {category.category1 && state.categories[category.category1] && (
            <div className="space-y-2">
              <div className="flex flex-wrap gap-2">
                {Object.keys(state.categories[category.category1]).map((category2) => {
