...
export const useProductUseCase = () => {
  ...
  const loadProductsAndCategories = useAutoCallback(async () => {
  ...
  const loadProducts = useAutoCallback(async (resetList = true) => {
    try {
      productStore.dispatch({
        type: PRODUCT_ACTIONS.SETUP,
  ...
  const loadMoreProducts = useAutoCallback(async () => {
  ...
  const searchProducts = useAutoCallback((search: string) => {
    router.query = { search, current: 1 };
  });

  const setCategory = useAutoCallback((categoryData: StringRecord) => {
    router.query = { ...categoryData, current: 1 };
  });

  const setSort = useAutoCallback((sort: string) => {
    router.query = { sort, current: 1 };
  });

  const setLimit = useAutoCallback((limit: number) => {
    router.query = { limit, current: 1 };
  });

  const loadProductDetailForPage = useAutoCallback(async (productId: string) => {
    try {
  ...
  const loadRelatedProducts = useAutoCallback(async (category2: string, excludeProductId: string) => {
    try {
      const params = {
        category2,
  ...
  const loadNextProducts = useAutoCallback(async () => {
