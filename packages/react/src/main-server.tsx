...
// SSR 전용 최소 컴포넌트들
const SSRHomePage = ({ products, totalCount, categories, query }: { 
  products: any[], 
  totalCount: number, 
  categories: any,
  ...
  const sortValue = query.sort || "price_asc"; // 기본값을 price_asc로 변경
  const category1 = query.category1 || "";
  const category2 = query.category2 || "";

  return createElement('div', { className: 'min-h-screen bg-gray-50' },
    createElement('header', { className: 'bg-white shadow-sm sticky top-0 z-40' },
      createElement('div', { className: 'max-w-md mx-auto px-4 py-4' },
    ...
    createElement('main', { className: 'max-w-md mx-auto px-4 py-4' },
      // SearchBar 컴포넌트 내용
      createElement('div', { className: 'mb-6' },
            ...
            Object.keys(categories).map((category) =>
              createElement('button', {
                ...
                className: `px-3 py-1 text-sm rounded-full border ${
                  category1 === category ? 'bg-blue-100 text-blue-800 border-blue-300' : 'bg-gray-100 text-gray-700 border-gray-300'
                }`
              }, category)
            )
          )
        ...
        // 브레드크럼 (카테고리가 선택된 경우)
        (category1 || category2) && createElement('div', { className: 'mb-4' },
          createElement('div', { className: 'text-sm text-gray-600' },
            '카테고리: ',
            category1 && createElement('button', {
        ...
        // 필터 옵션들
        createElement('div', { className: 'flex gap-2 items-center justify-between' },
          // 페이지당 상품 수
      ...
      // 상품 개수 정보
      createElement('div', { className: 'mb-4 text-sm text-gray-600' },
        '총 ', createElement('span', { className: 'font-medium text-gray-900' }, `${totalCount.toLocaleString()}개`), '의 상품'
      ),

      // 상품 그리드
      createElement('div', { className: 'grid grid-cols-2 gap-4 mb-6', id: 'products-grid' },
        products.slice(0, 20).map((product) =>
          createElement('div', { 
            key: product.productId,
            className: 'bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden product-card'
          },
            createElement('div', { className: 'aspect-square bg-gray-100 overflow-hidden cursor-pointer product-image' },
              createElement('img', {
                src: product.image,
                alt: product.title,
            ...
            createElement('div', { className: 'p-3' },
              createElement('div', { className: 'cursor-pointer product-info mb-3' },
                createElement('h3', { className: 'text-sm font-medium text-gray-900 line-clamp-2 mb-1' }, product.title),
                createElement('p', { className: 'text-xs text-gray-500 mb-2' }, product.brand),
                createElement('p', { className: 'text-lg font-bold text-gray-900' }, `${Number(product.lprice).toLocaleString()}원`)
              ),
              createElement('button', {
  ...
  return createElement('div', { className: 'min-h-screen bg-gray-50' },
    ...
    createElement('main', { className: 'max-w-md mx-auto px-4 py-4' },
      createElement('div', { className: 'mb-6' },
        ...
        createElement('div', { className: 'aspect-square bg-gray-100 rounded-lg overflow-hidden mb-4' },
          createElement('img', {
            src: product.image,
            alt: product.title,
        ...
        createElement('div', { className: 'mb-4' },
          createElement('p', { className: 'text-3xl font-bold text-gray-900 mb-2' }, `${Number(product.lprice).toLocaleString()}원`),
          createElement('p', { className: 'text-sm text-gray-500' }, product.brand)
        ),
        createElement('div', { className: 'flex items-center gap-4 mb-4' },
          createElement('button', { id: 'quantity-decrease', className: 'w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center' }, '-'),
          createElement('input', { 
            id: 'quantity-input',
            type: 'number',
            defaultValue: '1',
          ...
          createElement('button', { id: 'quantity-increase', className: 'w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center' }, '+')
        ),
        createElement('button', {
          id: 'add-to-cart-btn',
        ...
        createElement('div', { className: 'grid grid-cols-2 gap-4' },
          relatedProducts.slice(0, 4).map((relatedProduct) =>
            createElement('div', {
              ...
              className: 'bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden related-product-card cursor-pointer'
            },
              createElement('div', { className: 'aspect-square bg-gray-100 overflow-hidden' },
                createElement('img', {
              ...
              createElement('div', { className: 'p-3' },
                createElement('h3', { className: 'text-sm font-medium text-gray-900 line-clamp-2 mb-1' }, relatedProduct.title),
                createElement('p', { className: 'text-lg font-bold text-gray-900' }, `${Number(relatedProduct.lprice).toLocaleString()}원`)
              )
            )
          )
        )
...
export const render = async (url: string, query: Record<string, string>) => {
  ...
  try {
    ...
    if (url === "/" || url === "") {
      ...
      try {
        ...
        const filteredProducts = filterProducts(items, {
          ...
          sort: query.sort || "price_asc", // 기본값을 price_asc로 변경
        });
    ...
    const appHtml = renderToString(createElement(PageComponent, { ...initialData, query }));
