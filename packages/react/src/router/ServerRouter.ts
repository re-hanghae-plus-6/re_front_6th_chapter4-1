 ...
 */
type ServerRouteHandler = (params?: Record<string, string>) => Promise<unknown> | unknown;

export class ServerRouter extends Router<ServerRouteHandler> {
  #currentUrl = "/"; // 현재 처리 중인 요청 URL
  #origin = "http://localhost"; // 서버 origin
  #query: Record<string, string> = {}; // Express에서 파싱된 쿼리 객체
  ...
  findRoute(url: string) {
    ...
    const routes = (this as any)["#routes"] as Map<string, any>;
