...
const createTest = (BASE_URL: string) => {
  test.describe(`E2E: 쇼핑몰 전체 사용자 시나리오 (${BASE_URL})`, () => {
    ...
    test.describe("2. 검색 및 필터링 기능", () => {
      ...
      test("브레드크럼 클릭으로 상위 카테고리로 이동할 수 있다", async ({ page }) => {
        ...
        // 2차 카테고리 상태에서 시작
        await page.goto(`${BASE_URL}?current=1&category1=생활%2F건강&category2=자동차용품&search=차량용`);
        await helpers.waitForPageLoad();
    ...
    test.describe("3. 상태 유지 및 URL 복원", () => {
      test("검색어와 필터 조건이 URL에서 복원된다", async ({ page }) => {
        ...
        // 복잡한 쿼리 파라미터로 직접 접근
        await page.goto(`${BASE_URL}?search=젤리&category1=생활%2F건강&sort=price_desc&limit=10`);
        await helpers.waitForPageLoad();
    ...
    test.describe("8. SPA 네비게이션", () => {
      test("브라우저 뒤로가기/앞으로가기가 올바르게 작동한다", async ({ page }) => {
        ...
        await expect(page).toHaveURL(`${BASE_URL}product/85067212996/`);
        await expect(
          page.locator('h1:text("PVC 투명 젤리 쇼핑백 1호 와인 답례품 구디백 비닐 손잡이 미니 간식 선물포장")'),
        ).toBeVisible();
        await expect(page.locator("text=관련 상품")).toBeVisible();
        ...
        await expect(page).toHaveURL(`${BASE_URL}product/86940857379/`);
        await expect(
          page.locator('h1:text("샷시 풍지판 창문 바람막이 베란다 문 틈막이 창틀 벌레 차단 샤시 방충망 틈새막이")'),
        ).toBeVisible();
